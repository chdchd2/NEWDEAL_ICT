...
$(function(){
	$("#btnWrite").click(function(){
		location.href="<c:url value='/festival/write'/>";
	});
});
</script>
 ...
 <div id="subMenu">
					<h2>취업지원센터</h2>
					<ul>
						<li><a href="<c:url value='/festival/list'/>" class="subActive">정보공유 <img src="<c:url value='/resources/images/submenu_Active.png'/>" alt="서브메뉴활성화알림버튼"></a></li>
						<li><a href="#">행사안내</a></li>
						<li><a href="#a">프로그램</a></li>
					</ul>
				</div> 
<div id="sectionR">
...
<ul>
		<c:choose>
				<c:when test="${pu.startPageNum>9 }">
				 <li><a href="<c:url value='/festival/list?pageNum=${pu.startPageNum-1 }&fieldnum=${fieldnum }&classnum=${classnum }'/>"><span aria-hidden="true">&laquo;</span></a></li>
				</c:when>
				<c:otherwise>
		<li><a href="#"><span>&laquo;</span></a> </li>
				...
				<c:choose>
					<c:when test="${i==pu.pageNum }">
						<!-- 현재페이지 색상 다르게 표시하기 -->
						 <li class="active"><a href="<c:url value='/festival/list?pageNum=${i }&fieldnum=${fieldnum }&classnum=${classnum }'/>">${i }</a></li>
					</c:when>
					<c:otherwise>
					 <li><a href="<c:url value='/festival/list?pageNum=${i }&fieldnum=${fieldnum }&classnum=${classnum }'/>">${i }</a></li>
					</c:otherwise>
		...
		<c:choose>
			<c:when test="${pu.endPageNum<pu.totalPageCount }">
				  <li><a href="<c:url value='/festival/list?pageNum=${pu.endPageNum+1 }&classnum=${classnum }&fieldnum=${fieldnum }'/>" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
			</c:when>
			<c:otherwise>
